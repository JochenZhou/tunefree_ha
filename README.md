...
| 搜索数量限制 | 语音搜索返回的歌曲数量 | 否 |
| 播放器不能自动连续播放 | 启用播放进度监控，自动切歌 | 否 |

### 播放器兼容性说明

部分播放器（如 Music Player Daemon、小爱音箱等）在播放完一首歌曲后不会自动发送"idle"状态通知，导致无法自动切换到下一首歌曲。

如果您的播放器存在此问题，请在配置时勾选 **「播放器不能自动连续播放」** 选项。启用后，TuneFree 会每秒监控播放进度，在歌曲即将结束时自动切换到下一首。
...
TuneFree 支持通过 Home Assistant 的 AI 语音助手控制播放。
...
| 榜单类型 | 网易云 (netease) | 酷我 (kuwo) | QQ音乐 (qq) |
|---------|-----------------|-------------|-------------|
| 热歌榜 | 3778678 | 16 | 26 |
| 新歌榜 | 3779629 | 17 | 27 |
| 飙升榜 | 19723756 | 93 | 62 |
...
歌词卡片需要手动添加 Lovelace 资源：

1. 进入 **设置 → 仪表盘 → 右上角菜单 → 资源**
2. 点击「添加资源」
3. URL 填入：`/tunefree/tunefree-lyrics-card.js`
4. 类型选择：`JavaScript 模块`
